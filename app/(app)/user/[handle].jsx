...
const UserDetails = () => {
  ...
  const createdRides = rides.filter(r => r.creator && r.creator.handle === user.handle);

  const joinedRides = rides.filter(r => r.partners && r.partners.some(p => p && p.handle === user.handle));
